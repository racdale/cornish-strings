...
set.seed(2) # stable randomization seed for statistics in paper; patterns hold in all seeds explored
library(lme4)
...
[Fig. 4 PDF is generated in the supplementary-analysis folder.]

[Fig. 5 showing contrast in generation 1 and 10 in one chain]
...
#cTo = c('c','b','a','d','e','f')
# dev.off();dev.new()
# pdf(file="Figure5.pdf",width=70,height=70)
par(mfrow=c(1,2),oma = c(5,4,0,0) + 1,mar = c(0,0,1,1) + 1)
for (chain in 3) {
  # load this chain file
  strs = read.table(paste('data/chain',chain,'.chain',sep=''),sep='\t')  
  for (generation in c(1,11)) {
    ...
    plot(network,layout=layout.kamada.kawai,vertex.size=66,vertex.label=nodes,
         vertex.color=rgb(1,1,1),vertex.shape="rectangle",edge.width=weights,
         edge.color=rgb(0.25,0.25,0.25),
         vertex.label.color=rgb(0,0,0),vertex.label.cex=.7) 
    #text(x=layout.circle(network)[,1]*1.2,y=layout.circle(network)[,2]*1.1,labels=nodes)
  }
}
# dev.off()
...
[All networks using Kamada-Kawai force-directed algorithm for visualization; see Fig. S1 PDF]
